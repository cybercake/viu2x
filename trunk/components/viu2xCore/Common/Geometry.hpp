...
namespace v2x {
	...
	public:
		...
		Vector2D <T> & operator /= (const T & op) {
			...
			if (op == 0)
				throw Exception("Vector2D::/=: The operator should not be 0!");
		...
		inline Vector2D <T> operator + (const Vector2D <T> & op) const {
		...
		/// @param [in] op The scale factor
		/// @return result = this * op
		inline Vector2D <T> operator * (const T & op) const {
	...
	public:
		...
		Vector3D <T> & operator /= (const T & op) {
			...
			if (op == 0)
				throw Exception("Vector3D::/=: The operator should not be 0!");
		...
		/// @param [in] op The operand
		/// @return result = this + op
		inline Vector3D <T> operator + (const Vector3D <T> & op) const {
		...
		/// @param [in] op The scale factor
		/// @return result = this * op
		inline Vector3D <T> operator * (const T & op) const {
